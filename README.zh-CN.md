...
Codex、Claude Code、Pi 和 OpenCode 的全局 Instructions 由按顺序排列的 Markdown 片段声明。AGX 会锁定片段的精确内容，确定性拼接组合 Catalog 中启用的 Instructions，并且只管理各目标文件中的标记区块：
...
托管路径分别是 `$CODEX_HOME/AGENTS.md`（默认 `~/.codex/AGENTS.md`）、`$CLAUDE_CONFIG_DIR/CLAUDE.md`（默认 `~/.claude/CLAUDE.md`）、`$PI_CODING_AGENT_DIR/AGENTS.md`（默认 `~/.pi/agent/AGENTS.md`），以及 `$XDG_CONFIG_HOME/opencode/AGENTS.md`（默认 `~/.config/opencode/AGENTS.md`）。
