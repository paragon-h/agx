...
当前原型已经可以严格加载单个 `agx.yaml`、列出声明的 Skills、锁定本地或 Git 来源的 Skills、原子写入 `agx.lock`、通过 `agx lock --frozen` 验证锁定状态，并使用 `agx doctor` 诊断配置的 Codex 或 Claude Code target。Git branch、tag 和 commit 会通过系统 Git 解析为完整 commit SHA，并单独计算所选 Skill 子目录的摘要。安装类命令尚未实现。
